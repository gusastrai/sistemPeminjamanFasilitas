...
const DaftarPeminjamanBarang = () => {
  ...
  const fetchPeminjaman = async () => {
    try {
      setIsLoading(true);
      const data = await peminjamanBarangService.getAllPeminjamanBarangAcc();
          ...
          <TableBody>
            {peminjamanList.map((peminjaman, index) => (
              <TableRow key={peminjaman.idPeminjaman}>
                ...
                <TableCell>
                  {peminjaman.peminjamanBarang?.barang.namaBarang.charAt(0).toUpperCase() + peminjaman.peminjamanBarang?.barang.namaBarang.slice(1)}
                </TableCell>
                <TableCell>
